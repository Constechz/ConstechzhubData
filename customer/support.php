    ...
    if (!validateCSRF($_POST['csrf_token'])) {
        ...
        if (empty($subject) || empty($message)) {
            ...
            try {
                ...
                $stmt = $db->prepare("
                    INSERT INTO support_tickets (ticket_id, user_id, subject, message, priority, status, created_at) 
                    VALUES (?, ?, ?, ?, ?, 'open', NOW())
                ");
                $stmt->bind_param('sisss', $ticket_id, $current['id'], $subject, $message, $priority);
                ...
                if ($stmt->execute()) {
                    ...
                    $ticket_data = [
                        ...
                        'response_message' => 'New support ticket created: ' . $message
                    ];
                    
                    sendSupportTicketEmail($current['email'], $current['full_name'], $ticket_data);
                    
                    setFlashMessage('success', 'Support ticket created successfully. Ticket ID: ' . $ticket_id);
                } else {
                    setFlashMessage('error', 'Failed to create support ticket');
                }
            } catch (Exception $e) {
                error_log('Support ticket error: ' . $e->getMessage());
                setFlashMessage('error', 'An error occurred while creating your ticket');
            }
        }
    }
}
...
<html lang="en">
<head>
  <?php echo generateSeoMeta('Support', 'Get help and support for your Constechzhub account and services.'); ?>
    ...
    <main class="main-content">
      ...
      <div class="dashboard-content">
        <div class="dashboard-grid">
          <div class="widget">
            <div class="widget-header"><h3 class="widget-title">Create Ticket</h3></div>
              ...
              <form id="createTicketForm">
                ...
                <div class="form-group">
                  ...
                  <select class="form-control" id="category">
                    <option value="bundle_issue">Data bundle not delivered</option>
                    <option value="wallet_topup">Top-up wallet not credited</option>
                ...
                <button type="submit" class="btn btn-primary" id="createBtn"><i class="fas fa-paper-plane"></i> Submit Ticket</button>
              </form>
            </div>
          </div>
          <div class="widget">
            <div class="widget-header"><h3 class="widget-title">My Tickets</h3></div>
            <div class="widget-body">
              <div id="tickets"></div>
            </div>
          </div>
    ...
    <div class="modal-content">
      ...
      <div class="modal-body">
        ...
        <div class="reply-section">
          ...
          <textarea id="replyMessage" class="form-control" rows="3" placeholder="Type your reply here..."></textarea>
...
async function loadTickets() {
  ...
  if (data.status !== 'success') { container.innerHTML = '<div class="alert alert-danger">Failed to load tickets</div>'; return; }
  if (!data.tickets.length) { container.innerHTML = '<div class="text-muted">No tickets yet</div>'; return; }
...
async function openThread(id) {
  currentTicketId = id;
  const messagesRes = await fetch('../api/support.php?action=list_messages&ticket_id=' + id);
  ...
  // Update modal title
  document.getElementById('modalTitle').textContent = `Ticket #${id} - Messages`;
