...
function sendAgentOrderDeliveredSms($order_id) {
    global $db;
    
    $isEnabled = getSetting('agent_delivery_sms_enabled', '0');
    if ($isEnabled !== '1') {
        return ['success' => false, 'error' => 'SMS notification is disabled'];
    }
    
    // Fetch order details
    $stmt = $db->prepare("
        SELECT bo.order_reference, bo.beneficiary_number, bo.amount, bo.agent_id,
               dp.data_size, n.name AS network_name,
    ...
    ");
    if (!$stmt) {
        return ['success' => false, 'error' => 'Database error preparing query'];
    }
    ...
    if (!$order) {
        return ['success' => false, 'error' => 'Order or agent not found'];
    }
    
    if (empty($order['agent_phone'])) {
        return ['success' => false, 'error' => 'Agent phone number is missing'];
    }
    
    $defaultTemplate = 'Hi {agent_name}, order {reference} for {beneficiary_number} ({data_size} {network}) has been delivered successfully. {site_name}';
    $template = getSetting('agent_delivery_sms_template', $defaultTemplate);
    if (trim((string)$template) === '') {
        $template = $defaultTemplate;
    }
    
    $site_name = function_exists('getSiteName') ? getSiteName() : 'Constechzhub';
    $amount_formatted = function_exists('formatCurrency') ? formatCurrency($order['amount']) : number_format($order['amount'], 2);
    
    // Replace placeholders
    $message = str_replace(
        ['{agent_name}', '{reference}', '{beneficiary_number}', '{data_size}', '{network}', '{site_name}', '{amount}'],
        [
            $order['agent_name'],
    ...
    return sendSMS($order['agent_phone'], $message, 'agent_delivery', $order['agent_id']);
}

function sendAgentStoreOrderCompletedSms($order_id) {
    global $db;
    
    $isEnabled = getSetting('agent_store_delivery_sms_enabled', '0');
    if ($isEnabled !== '1') {
        return ['success' => false, 'error' => 'SMS notification is disabled'];
    }
    
    // Fetch order details, verifying it is a store link purchase (agent_id exists and agent_id != user_id)
    $stmt = $db->prepare("
        SELECT bo.order_reference, bo.beneficiary_number, bo.amount, bo.agent_id, bo.user_id,
               dp.data_size, n.name AS network_name,
               u.full_name AS agent_name, u.phone AS agent_phone
        FROM bundle_orders bo
        JOIN data_packages dp ON bo.package_id = dp.id
        JOIN networks n ON dp.network_id = n.id
        JOIN users u ON bo.agent_id = u.id
        WHERE bo.id = ? AND bo.agent_id IS NOT NULL AND bo.agent_id != bo.user_id
        LIMIT 1
    ");
    if (!$stmt) {
        return ['success' => false, 'error' => 'Database error preparing query'];
    ...
    if (!$order) {
        return ['success' => false, 'error' => 'Store order or agent not found'];
    }
    ...
    $defaultTemplate = 'Hi {agent_name}, an order ({reference}) of {data_size} {network} placed via your store link has been successfully completed. {site_name}';
    $template = getSetting('agent_store_delivery_sms_template', $defaultTemplate);
    if (trim((string)$template) === '') {
        $template = $defaultTemplate;
    }
    ...
    // Replace placeholders
    $message = str_replace(
        ['{agent_name}', '{reference}', '{data_size}', '{network}', '{beneficiary_number}', '{site_name}'],
        [
            $order['agent_name'],
            $order['order_reference'],
            $order['data_size'],
    ...
    return sendSMS($order['agent_phone'], $message, 'agent_store_delivery', $order['agent_id']);
}
