 ...
 */
function processBundlePurchase($bundle_order_id, $network_id, $phone, $volume_gb, $endpoint_type = 'regular') {
    global $db;
    ...
    try {
        $result = callProviderAPI($provider['provider_slug'], $network_id, $phone, $volume_gb, $bundle_order_id, $resolved_endpoint_type);
        ...
        // Log the API call
        logAPITransaction($bundle_order_id, $provider['provider_id'], $network_id, $resolved_endpoint_type, 
                         $result['request_data'] ?? [], $response_data, $result['http_code'] ?? 0, 
                         $result['response_time'] ?? 0, $success ? 1 : 0, $error_message, 0);
        
    } catch (Exception $e) {
        $error_message = $e->getMessage();
        logAPITransaction($bundle_order_id, $provider['provider_id'], $network_id, $resolved_endpoint_type,
                         [], null, 0, 0, 0, $error_message, 0);
    }
    
    // Try backup provider if primary failed
    if (!$success) {
        $backup_provider = getBackupProvider($network_id, $resolved_endpoint_type);
        if ($backup_provider && (int)$backup_provider['provider_id'] !== (int)$provider['provider_id']) {
            $backup_endpoint_type = $backup_provider['resolved_endpoint_type'] ?? $resolved_endpoint_type;
            try {
                $result = callProviderAPI($backup_provider['provider_slug'], $network_id, $phone, $volume_gb, $bundle_order_id, $backup_endpoint_type);
                $success = $result['success'];
                $response_data = $result['response'];
                $error_message = $result['error'] ?? null;
                $reference = $result['reference'] ?? extractProviderReference($response_data);
                if ($success) {
                    $successful_provider = $backup_provider;
                }
                
                // Log the backup API call
                logAPITransaction($bundle_order_id, $backup_provider['provider_id'], $network_id, $backup_endpoint_type,
                                $result['request_data'] ?? [], $response_data, $result['http_code'] ?? 0,
                                $result['response_time'] ?? 0, $success ? 1 : 0, $error_message, 1);
                
            } catch (Exception $e) {
                $error_message = $e->getMessage();
                logAPITransaction($bundle_order_id, $backup_provider['provider_id'], $network_id, $backup_endpoint_type,
                                [], null, 0, 0, 0, $error_message, 1);
            }
        }
    }
    ...
    return [
        'success' => $success,
        'provider' => $success ? $successful_provider : $provider,
 ...
 */
function callProviderAPI($provider_slug, $network_id, $phone, $volume_gb, $bundle_order_id, $endpoint_type) {
    ...
    // Convert to 10-digit local format (0xxxxxxxxx) for all providers
    if (strlen($phone) == 12 && substr($phone, 0, 3) == '233') {
        // Convert international format (233xxxxxxxxx) to local (0xxxxxxxxx)
        $formatted_phone = '0' . substr($phone, 3);
    } elseif (strlen($phone) == 10 && substr($phone, 0, 1) == '0') {
        // Already in correct local format (0xxxxxxxxx)
        $formatted_phone = $phone;
    } elseif (strlen($phone) == 9) {
        // Add leading zero if missing (xxxxxxxxx -> 0xxxxxxxxx)
        $formatted_phone = '0' . $phone;
    }
    
    // Get buyer phone number (referrer) from order user
    $buyer_phone = '';
    if ($bundle_order_id > 0) {
        ...
        if ($order_stmt) {
            ...
            if ($order_res) {
                $buyer_phone = $order_res['phone'] ?? '';
            }
            $order_stmt->close();
        }
    }
    
    // Format buyer phone number to be 10 digits
    if (!empty($buyer_phone)) {
        if (strlen($buyer_phone) == 12 && substr($buyer_phone, 0, 3) == '233') {
            $buyer_phone = '0' . substr($buyer_phone, 3);
        } elseif (strlen($buyer_phone) == 9) {
            $buyer_phone = '0' . $buyer_phone;
        }
    }

    // Replace placeholders in request format
    $reference = ($provider['provider_name'] === 'Hubnet Console')
        ? generateReference('HUBNET', 'integer')
        : generateReference('API');
    $request_data = [];
    foreach ($request_format as $key => $value) {
        $request_data[$key] = replacePlaceholders($value, [
            ...
            'network' => strtolower($network_name),
            'webhook' => SITE_URL . '/api/hubnet_webhook.php',
            'referrer' => $buyer_phone ?: $formatted_phone
        ]);
    }
 ...
 */
function detectEndpointTypeForPackage($package_name = '', $package_data_size = '') {
    $needle = strtolower(trim((string)$package_name . ' ' . (string)$package_data_size));
    if ($needle === '') {
        return 'regular';
    }
    if (strpos($needle, 'bigtime') !== false || strpos($needle, 'big time') !== false) {
        return 'bigtime';
    }
...
function isResponseSuccessful($response_data, $response_format, $http_code) {
    ...
    return $current_data == $success_value;
}
...
function extractErrorMessage($response_data, $response_format) {
    ...
    if (is_array($response_format) && isset($response_format['error_field'])) {
        $error_field = $response_format['error_field'];
        $field_parts = explode('.', $error_field);
        $current_data = $response_data;
