...
if ($lookup !== '') {
    ...
    $where = "(bo.agent_id = ? OR bo.user_id = ?) AND (" . implode(" OR ", $filters) . ")";
    ...
    if ($phone_lookup !== null) {
        $stmt->bind_param("iiss", $current_user['id'], $current_user['id'], $phone_lookup, $reference);
    } else {
        $stmt->bind_param("iis", $current_user['id'], $current_user['id'], $reference);
    }
    $stmt->execute();
    $rs = $stmt->get_result();
...
function dbh_agent_reference_phone_display($phone) {
                                ...
                                <?php foreach ($results as $order): ?>
                                    ...
                                    <div class="lookup-card">
                                        ...
                                        <div class="lookup-meta">
                                            ...
                                            <div><strong>Phone:</strong> <?php echo htmlspecialchars(dbh_agent_reference_phone_display($order['beneficiary_number'])); ?></div>
