        ...
        if ($id > 0 && $name && $network && $type && $size && $validity > 0) {
            ...
            if ($networkRow = $networkResult->fetch_assoc()) {
                $stmt = $db->prepare("
                    ...
                    SET name = ?, network_id = ?, package_type = ?, data_size = ?, validity_days = ?
                    WHERE id = ?
                ");
                $stmt->bind_param('sissii', $name, $networkRow['id'], $type, $size, $validity, $id);
                            ...
                            <?php else: ?>
                                <?php foreach ($packages as $pkg): ?>
                                    <tr>
                                        ...
                                        <td data-label="Package Name"><?php echo htmlspecialchars($pkg['name']); ?></td>
                                        ...
                                        <td data-label="Actions">
                                            <a href="packages.php?edit_id=<?php echo (int)$pkg['id']; ?>" class="btn btn-primary btn-sm" title="Edit Package" style="margin-right: .35rem;">
