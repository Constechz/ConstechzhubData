...
$local_phone = normalizeAtLocalPhone($beneficiary_number);
if (!isAtLocalPhone($local_phone)) {
    http_response_code(400);
    echo json_encode(['status' => 'error', 'message' => 'Use AirtelTigo numbers only (026/027/056/057) and 10 digits.']);
    exit();
}
...
$stmt = $db->prepare('
    ...
    WHERE dp.id = ? AND dp.status = "active" AND COALESCE(dp.stock_status, "in_stock") = "in_stock" AND n.name = "AT"
');
...
$description = 'Agent bundle purchase: AT ' . ($package['data_size'] ?? $package['name'] ?? 'bundle') . ' for ' . $formatted_phone;
...
$metadata = [
    ...
    'return_to' => '/agent/at-business.php'
];
...
try {
    ...
    error_log('Agent Paystack Init Exception: ' . $e->getMessage());
