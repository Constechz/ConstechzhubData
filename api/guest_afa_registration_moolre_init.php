...
if (!isPaymentGatewayEnabled('moolre')) {
    guest_afa_error('Moolre is currently disabled by admin settings.');
}

$config = getMoolreConfig();
if (!isMoolreConfigured($config)) {
    guest_afa_error('Moolre keys are not configured.');
}
...
if ((int) ($settings['allow_guest_moolre'] ?? 1) !== 1) {
    guest_afa_error('Guest Moolre checkout is disabled.');
}
...
$stmt = $db->prepare("INSERT INTO transactions (user_id, transaction_type, amount, status, reference, payment_method, description, metadata) VALUES (?, 'purchase', ?, 'pending', ?, 'moolre', ?, ?)");
$stmt->bind_param('idsss', $user_id, $guest_price, $reference, $description, $metadata_json);
$stmt->execute();

$agent_sql = $agent_id > 0 ? (string) $agent_id : 'NULL';
$stmt = $db->prepare("INSERT INTO afa_registrations (user_id, agent_id, beneficiary_name, email, phone, ghana_card_number, ghana_card_front_image, ghana_card_back_image, location, occupation, region, date_of_birth, amount, admin_price, profit_amount, payment_gateway, reference, status) VALUES (?, {$agent_sql}, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, 'moolre', ?, 'pending')");
...
if (!$result) {
    guest_afa_error($error ?: 'Moolre initialization failed', 500);
}

$status_ok = isset($result['status']) && ((int) $result['status'] === 1 || $result['status'] === true);
if (!$status_ok) {
    guest_afa_error($result['message'] ?? 'Moolre initialization failed', 500);
}

$auth_url = $result['data']['authorization_url'] ?? '';
if ($auth_url === '') {
    guest_afa_error('Missing authorization URL', 500);
}
