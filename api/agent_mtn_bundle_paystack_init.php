...
$debug_log = __DIR__ . '/../debug_paystack.log';
...
if (!validatePhone($beneficiary_number)) {
    file_put_contents($debug_log, "ERROR: Invalid Phone: $beneficiary_number\n", FILE_APPEND);
    http_response_code(400);
    echo json_encode(['status' => 'error', 'message' => 'Please enter a valid phone number']);
    exit();
}

$local_phone = normalizeMtnLocalPhone($beneficiary_number);
if (!isMtnLocalPhone($local_phone) && !($allow_ported_mtn && preg_match('/^\d{10}$/', $local_phone))) {
    file_put_contents($debug_log, "ERROR: Not MTN Phone: $local_phone\n", FILE_APPEND);
    http_response_code(400);
    echo json_encode(['status' => 'error', 'message' => 'Use MTN numbers only (024/025/053/054/055/059), or confirm that this number has been ported to MTN.']);
    exit();
}
...
$stmt = $db->prepare('
    ...
    WHERE dp.id = ? AND dp.status = "active" AND COALESCE(dp.stock_status, "in_stock") = "in_stock" AND n.name = "MTN"
');
...
if (!$package) {
    file_put_contents($debug_log, "ERROR: Package Not Found or Inactive. ID: $package_id\n", FILE_APPEND);
...
if ($price_to_charge <= 0) {
    file_put_contents($debug_log, "ERROR: Invalid Price: $price_to_charge\n", FILE_APPEND);
    http_response_code(400);
...
if ($duplicate_order) {
    file_put_contents($debug_log, "ERROR: Duplicate Order. Ref: " . ($duplicate_order['order_reference'] ?? 'none') . "\n", FILE_APPEND);
...
if ($recent_duplicate) {
    file_put_contents($debug_log, "ERROR: Recent Guest Duplicate. Ref: " . ($recent_duplicate['reference'] ?? 'none') . "\n", FILE_APPEND);
    http_response_code(409);
    echo json_encode([
        'status' => 'error',
...
if (!$availability['available']) {
    file_put_contents($debug_log, "ERROR: Provider Unavailable: " . ($availability['message'] ?? 'unknown') . "\n", FILE_APPEND);
    http_response_code(400);
...
if (!isConfiguredPaystackSecretKey($paystack_secret_key)) {
    file_put_contents($debug_log, "ERROR: Paystack Secret Key Missing or Invalid\n", FILE_APPEND);
    http_response_code(400);
...
$description = 'Agent bundle purchase: MTN ' . ($package['data_size'] ?? $package['name'] ?? 'bundle') . ' for ' . $formatted_phone;
$customer_email = trim((string) ($current_user['email'] ?? ''));
if ($customer_email === '' || !validateEmail($customer_email)) {
    file_put_contents($debug_log, "ERROR: Invalid Email: $customer_email\n", FILE_APPEND);
    http_response_code(400);
...
$metadata = [
    ...
    'return_to' => '/agent/mtn-business.php'
];
